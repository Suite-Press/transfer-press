...
class AdminMenuHandlers {
    ...
    {
        add_action('admin_menu', [$this, 'transferPressAddAdminMenu']);
        add_action('admin_enqueue_scripts', [$this, 'conditionallyEnqueueAssets']);
    }

    public function conditionallyEnqueueAssets($hook): void
    {

        // Only load assets on our plugin page
        if (
            strpos($hook, 'transfer-press-export') === false &&
            strpos($hook, 'transfer-press-import') === false
        ) {
            return;
        }
        $this->transferPressEnqueueAssets();
    }

    public function transferPressAddAdminMenu(): void
    {
        ...
        add_menu_page(
            ...
            __('Transfer Press', 'transfer-press'),
            'manage_options',
            'transfer-press-export',
            [$this, 'renderExportDashboard'],
            $this->getMenuIcon(),
            26
        );
        add_submenu_page(
...
<path d="M23.6179 10.1547C23.8948 10.3449 24.0411 10.6751 23.9906 11.0061L22.2895 22.3861C22.2496 22.6514 22.0893 22.8845 21.8547 23.0167C21.6202 23.1489 21.3387 23.1666 21.0895 23.0637L17.8141 21.719L15.9501 23.7527C15.7079 24.0189 15.3255 24.108 14.9887 23.9782C14.6519 23.8484 14.4318 23.524 14.4303 23.1632L14.4205 20.8777C14.4201 20.7684 14.4606 20.6643 14.5341 20.582L19.0946 15.5651C19.2524 15.3922 19.2458 15.1271 19.0811 14.9637C18.9164 14.8004 18.6512 14.7906 18.4796 14.9472L12.9401 19.9108L10.5211 18.7128C10.2307 18.5691 10.0435 18.2801 10.0339 17.9576C10.0244 17.635 10.1926 17.3336 10.4707 17.1711L22.6883 10.1204C22.9801 9.95241 23.341 9.96727 23.6179 10.1547Z" fill="#CFCFDE"/>
    ...
    public function renderExportDashboard(): void
    {
        include_once TRANSFER_PRESS_PATH. '/app/Views/export-admin-vue.php';
    }
    public function renderImportDashboard(): void
    {
        include_once TRANSFER_PRESS_PATH. '/app/Views/import-admin-vue.php';
    }
    public function transferPressEnqueueAssets(): void
    {

        $dev_server = 'http://localhost:5173';
        $hot_file_path = TRANSFER_PRESS_PATH . '/.hot';
        ...
        if ($is_dev) {
            ...
            wp_localize_script('my-plugin-vite', 'TransferPressSettings', [
                'nonce'   => wp_create_nonce('wp_transfer_press_setting'),
                'ajaxurl' => admin_url('admin-ajax.php'),
            ]);
        } else {
            // Prod: Use filemtime for cache busting
            $main_js = TRANSFER_PRESS_BUILD_PATH . '/main.js';
            $main_css = TRANSFER_PRESS_BUILD_PATH . '/main.css';

            $js_version = file_exists($main_js) ? filemtime($main_js) : '1.0.0';
            $css_version = file_exists($main_css) ? filemtime($main_css) : '1.0.0';

            wp_enqueue_script('my-plugin-main', TRANSFER_PRESS_BUILD_URL . '/main.js', [], $js_version, true);
            wp_enqueue_style('my-plugin-style', TRANSFER_PRESS_BUILD_URL . '/main.css',[],$css_version);

            wp_localize_script('my-plugin-main', 'TransferPressSettings', [
                'nonce'   => wp_create_nonce('wp_transfer_press_setting'),
                'ajaxurl' => admin_url('admin-ajax.php'),
            ]);
        }
